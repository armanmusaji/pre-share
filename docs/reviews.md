...
## Concept review
Claude's biggest change: it narrowed the scope. The person chooses one of three answers (Confirmed, Conditional, Not decided), and the proposed repair can change only the items linked to that decision. That made it possible to check whether anything unrelated changed. Claude also recommended keeping live AI in scope with tighter limits, which Arman later cut.
...
It also found problems Arman had already caught once before: a mark that looked like a checkbox but did nothing, and demo wording inside the product. It also caught wording that gave Sam authority the transcript did not establish.
